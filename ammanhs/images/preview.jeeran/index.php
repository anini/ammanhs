...
if (!file_exists($abs_dst)) {
  ...
  if (Conf::$copy_to && is_dir(Conf::$copy_to) && $bucketName) {
    ...
    // Use S3 PHP API
    //if (!copy($abs_dst, $dup_dst)) error('could not save image copy');
    $s3 = new S3($awsAccessKey, $awsSecretKey, false, 's3-eu-west-1.amazonaws.com');
    $my_header = array('Cache-Control' => 'max-age=2592000,public');
    // Put our file (also with public read access)
    $s3->putObjectFile($abs_dst, $bucketName, $dup_dst0, S3::ACL_PUBLIC_READ, array(), $my_header);
    
    // FIXME: in case of error should we remove the $abs_dst
  }
