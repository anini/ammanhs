...
class CSolrComponent extends CApplicationComponent{
    ...
    */
    ...
    // httpTransport can be "Curl", "CurlNoReuse" or "FileGetContents"
    public $httpTransport = "Curl";
    ...
    public function init()
    {
        ...
        if(!$this->host || !$this->indexPath)
            ...
            if ($this->httpTransport) {
                $trans_class="Apache_Solr_HttpTransport_".$this->httpTransport;
                require_once(dirname(__FILE__) . "/phpSolrClient/HttpTransport/{$this->httpTransport}.php");
                $trans = new $trans_class();
            } else {
                $trans=false;
            }
            $this->_solr = new Apache_Solr_Service($this->host, $this->port, $this->indexPath, $trans);
        }
        if (!$this->_solr->ping()){
    ...
    */
     ...
     * Commit Indexed Data
     */
    public function solrCommit($optimize=true){
       $this->_solr->commit();
       if ($optimize) $this->_solr->optimize();
       return true;
    }
    /**
