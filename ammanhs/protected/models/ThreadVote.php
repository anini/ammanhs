 ...
 *
 * The followings are the available columns in table '{{thread_vote}}':
 * @property integer $id
...
class ThreadVote extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_id, thread_id, vote_type, created_at, updated_at', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, user_id, thread_id, vote_type, created_at, updated_at', 'safe', 'on'=>'search'),
		);
	}
	...
	public function afterSave()
	{
		...
		if(!$this->isNewRecord){
			if(!UserLog::removeActivity($old_vote_type, $this->thread, $this->user))
