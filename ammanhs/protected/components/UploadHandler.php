...
class UploadHandler
{
    ...
    function __construct($upload_url, $options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            'file_name' => false,
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).$upload_url,
    ...
    protected function get_file_name($name,
        ...
        if ($this->options['file_name']) {
            $name = preg_replace('/.+?(\.[^.]*$|$)/', $this->options['file_name'].'$1', $name);
        }
