...
class Text
{
    ...
    public static function safe_b64_encode($s) {
        return str_replace(array('+', '/', '='), array('-', '_', ''), base64_encode($s));
    }

    public static function safe_b64_decode($s) {
        $pad=4 - (strlen($s) % 4);
        if ($pad==1) $s=$s."=";
        else if ($pad==2) $s=$s."==";
        else if ($pad==3) $s=$s."===";
    ...
    static public function squeze_whitespaces($text, $single_line=false) {
      if ($single_line) {
...
*/
    ...
    public static function language_detect($text){
          $a=array();
      if (!preg_match("/\p{L}/u", html_entity_decode($text), $a) || sizeof($a)<1) return Constants::LANG_UNSET;
      $l=$a[0];
      if (preg_match("/\p{Arabic}/u", $l)) return Constants::LANG_AR;
      if (preg_match("/\p{Latin}/u", $l)) return Constants::LANG_EN;
      return Constants::LANG_UNSET;
    }
