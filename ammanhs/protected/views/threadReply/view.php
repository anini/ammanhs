		...
		<div class="shadow-box thread-reply">
			<div>
				...
				<div class="reply-vote" id="reply-vote-<?php echo $model->id; ?>">
					<div class="arrow vote-up vote-up-<?php echo ((!$is_guest && $model->my_up_vote)?'on':'off'); ?>" title="Vote this reply up!" onclick="$('#thread-reply-vote-type').val(1); $('#thread-reply-vote-id').val(<?php echo $model->id; ?>); $('#thread-reply-vote-form').submit();"></div>
					<div class="vote-number"><?php echo $model->stat_votes; ?></div>
					<div class="arrow vote-down vote-down-<?php echo ((!$is_guest && $model->my_down_vote)?'on':'off'); ?>" title="Vote this reply down!" onclick="$('#thread-reply-vote-type').val(-1); $('#thread-reply-vote-id').val(<?php echo $model->id; ?>); $('#thread-reply-vote-form').submit();"></div>
				</div>
			</div>
		</div>
	</div>
